...
Ce script entraîne un adaptateur LoRA sur le U-Net de SD 1.5 en
utilisant des prompts structurés qui encodent les paramètres physiques
mesurés (caméra, heure, luminosité, intensité lave, pente, météo).

Le modèle apprend ainsi à respecter les conditions physiques fournies
plutôt que de simplement reproduire des textures.

Fonctionnalités :
  - Prompts structurés avec paramètres physiques dérivés de l'index
  - Negative prompt anti-hallucinations constant pendant l'entraînement
  - Classifier-Free Guidance (CFG) training avec drop conditionnel
  - LoRA rank configurable (4 ou 8)
  - Textual Inversion optionnelle (<merapi>)
  - EMA, gradient accumulation, early stopping
  - Génération de test avec paramètres physiques spécifiques
...
Usage :
    ...
    # Colab (CUDA) — entraînement complet
    python train_lora_physics.py --epochs 100 --resolution 256
...
# ============================================================
# Constantes — Prompts & Paramètres physiques
# ============================================================

# Negative prompt anti-hallucinations (utilisé pendant l'entraînement)
NEGATIVE_PROMPT = (
    "extra volcanoes, duplicate peaks, fantasy shapes, glowing rivers, "
    "buildings, people, text, artifacts, wrong perspective, unnatural colors, "
    "cartoon, anime, drawing, watermark, oversaturated, blurry"
)

# Template de prompt structuré
PROMPT_TEMPLATE = (
    "Merapi, single lava flow, camera={camera}, time={time_of_day}, "
    "brightness={brightness}, lava_intensity={lava_intensity}, "
    "slope={slope}, weather={weather}. "
    "Realistic surveillance view. No extra volcanoes."
)

# Prompt vide pour le CFG training (10% du temps)
UNCOND_PROMPT = ""

# Mapping des plages horaires
def _time_of_day(hour: int) -> str:
    """Convertit une heure en catégorie temporelle."""
    if 6 <= hour < 10:
...
def _brightness_from_hour_and_anomaly(hour: int, anomaly: float) -> str:
    """Estime la luminosité à partir de l'heure et du score d'anomalie."""
    if 6 <= hour < 18:
        if anomaly > 0.3:
            return "bright_with_incandescence"
        return "daylight"
    else:
        if anomaly > 0.3:
...
def _lava_intensity(anomaly: float) -> str:
    """Convertit le score d'anomalie en intensité de lave qualitative."""
    if anomaly > 0.5:
        return "very_high"
    elif anomaly > 0.3:
        return "high"
...
def _camera_from_filename(filename: str) -> str:
    """Extrait le nom de la caméra depuis le filename."""
    fn = filename.lower()
...
def _slope_from_camera(camera: str) -> str:
    """Estime la pente visible selon la caméra (angle de vue connu)."""
    # Les caméras VELI ont des angles de vue fixes sur le Merapi
    slopes = {"Suki": "30deg_south", "Kalor": "25deg_west", "Kali": "35deg_east"}
    return slopes.get(camera, "30deg_unknown")


def _weather_from_quality_and_hour(quality_flag: str, hour: int) -> str:
    """Estime la météo depuis le flag qualité et l'heure."""
    if quality_flag == "cloudy":
        return "overcast"
    elif quality_flag == "dark":
        return "clear_night" if hour >= 18 or hour < 6 else "hazy"
    return "clear"


def build_structured_prompt(row: dict, use_token: str | None = None) -> str:
    """Construit un prompt structuré à partir des métadonnées d'une image."""
    hour = int(row.get("hour", 12))
    anomaly = float(row.get("anomaly_score", 0.0))
    filename = str(row.get("filename", ""))
    quality = str(row.get("quality_flag", "usable"))

    camera = _camera_from_filename(filename)
    tod = _time_of_day(hour)
    brightness = _brightness_from_hour_and_anomaly(hour, anomaly)
    lava_int = _lava_intensity(anomaly)
    slope = _slope_from_camera(camera)
    weather = _weather_from_quality_and_hour(quality, hour)
    ...
    if use_token:
        prompt = f"{use_token} {prompt}"

    return prompt


# ============================================================
# 1. Dataset Merapi avec paramètres physiques
# ============================================================

class MerapiPhysicsDataset(Dataset):
    """
    Dataset PyTorch qui charge les images Merapi et construit
    des prompts structurés encodant les paramètres physiques.

    Chaque échantillon contient :
      - pixel_values : tensor [3, H, W] dans [-1, 1]
      - prompt       : prompt structuré avec paramètres physiques
      - metadata     : dict des paramètres extraits
    """
    ...
    def __init__(
        ...
        # Filtrer les nuits sans incandescence si demandé
        ...
        for _, row in df.iterrows():
            ...
            prompt = build_structured_prompt(row.to_dict(), use_token=use_token)
            ...
            self.samples.append({
                ...
                "camera": _camera_from_filename(fn),
                "filename": fn,
            })
...
def setup_lora_unet(unet, lora_rank: int = 4, lora_alpha: int = 4):
    ...
    config = LoraConfig(
        ...
        target_modules=[
            ...
            "proj_in", "proj_out",
            # Cross-attention layers — critiques pour le conditionnement texte
    ...
    print(f"[LoRA] {trainable:,} / {total:,} params entraînables ({100*trainable/total:.2f}%)")
...
def encode_prompt(prompt: str, tokenizer, text_encoder, device) -> torch.Tensor:
    """Encode un prompt texte en embedding."""
    tokens = tokenizer(
...
# ============================================================
# 3. Génération de test avec paramètres physiques
# ============================================================

# Scénarios de test couvrant les combinaisons de paramètres
TEST_SCENARIOS = [
    {
        "name": "jour_calme_suki",
        "params": {
...
def generate_physics_samples(
    ...
    for scenario in scenarios:
        ...
        print(f"  [{prefix}] {name}: {prompt[:80]}...", flush=True)
        with torch.no_grad():
            result = pipe(
                prompt,
...
def train(args: argparse.Namespace) -> None:
    ...
    # Encoder aussi le prompt vide (pour CFG training)
    uncond_emb = encode_prompt(UNCOND_PROMPT, tokenizer, text_encoder, device)
    # Encoder le negative prompt (pour CFG training)
    neg_emb = encode_prompt(NEGATIVE_PROMPT, tokenizer, text_encoder, device)
    ...
    # Montrer un exemple de prompt
    example = dataset.samples[0]
    print(f"\n[Exemple prompt] {example['prompt']}")
    ...
    if not args.skip_gen:
        print("\n[Pré-FT] Génération avec paramètres physiques...")
        ...
        generate_physics_samples(
            pre_pipe, TEST_SCENARIOS, samples_dir / "before_ft",
            prefix="before", seed=42,
        )
        del pre_pipe
        if device.type == "cuda":
    ...
    # ==========================================================
    print(f"\n{'='*60}")
    print(f"ENTRAÎNEMENT LoRA PHYSICS — {args.epochs} epochs")
    print(f"{'='*60}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # 10% du temps : prompt vide (apprend le unconditional)
            # 90% du temps : prompt structuré
            ...
            for p in prompts:
                if rng_cfg.random() < args.cfg_dropout:
                    ...
                    if p in _prompt_cache:
                        ...
                        emb = encode_prompt(p, tokenizer, text_encoder, device)
                        _prompt_cache[p] = emb
    ...
    # Sauvegarder les métadonnées de config
    ...
    with open(config_path, "w") as f:
        ...
        f.write(f"prompt_template: {PROMPT_TEMPLATE}\n")
    ...
    # ==========================================================
    if not args.skip_gen:
        print("\n[Post-FT] Génération avec paramètres physiques...")
        unet.eval()
        unet_raw = _unwrap_unet(unet)
        ...
        generate_physics_samples(
            post_pipe, TEST_SCENARIOS, samples_dir / "after_ft",
            prefix="after", seed=42,
        )
        del post_pipe
    else:
    ...
    print(f"\nPour évaluer : python evaluate_lora_physics.py")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="LoRA SD 1.5 avec conditionnement physique pour le Merapi"
    )

    # Dataset
    p.add_argument("--resolution", type=int, default=256)
    p.add_argument("--max-images", type=int, default=500)
    p.add_argument("--min-anomaly", type=float, default=0.0,
    ...
    p.add_argument("--cfg-dropout", type=float, default=0.1,
                   help="Probabilité de remplacer le prompt par '' (CFG training)")

    # LoRA
    p.add_argument("--lora-rank", type=int, default=4)
    p.add_argument("--lora-alpha", type=int, default=4)
